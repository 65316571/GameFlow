  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-title">游戏类型百科</div>
        ...
        {GENRES.map(g => {
          ...
          return (
            <div key={g.code} className="card"
              style={{ cursor: 'pointer', border: `0.5px solid ${open ? '#afa9ec' : '#e8e6df'}` }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{
                  ...
                  fontSize: g.code === 'MOBA' ? 10 : 12, fontWeight: 700,
                }}>
                  {g.code}
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>{g.name}</div>
                  <div style={{ fontSize: 11, color: '#b4b2a9', marginTop: 1 }}>{g.full}</div>
                </div>
                <div style={{ marginLeft: 'auto', fontSize: 12, color: '#b4b2a9' }}>{open ? '↑' : '↓'}</div>
              </div>

              <div style={{ fontSize: 12, color: '#888780', lineHeight: 1.7 }}>{g.desc}</div>

              {open && (
                <div onClick={e => e.stopPropagation()}>
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: '0.5px solid #e8e6df' }}>
                    <div className="section-label" style={{ marginBottom: 6 }}>代表游戏</div>
